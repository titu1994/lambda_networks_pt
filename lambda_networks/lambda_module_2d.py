...
class LambdaLayer2D(nn.Module):
    def __init__(
        ...
        self.to_k = nn.Conv2d(dim, dim_k * dim_intra_dimension, 1, bias=False)
        self.to_v = nn.Conv2d(dim, dim_v * dim_intra_dimension, 1, bias=False)
        ...
        self.norm_v = nn.BatchNorm2d(dim_v * dim_intra_dimension)
        ...
        if r is not None:
            ...
            if self.implementation == 0:
                self.pos_conv = nn.Conv3d(dim_intra_dimension, dim_k, (1, r, r), padding=(0, r // 2, r // 2))
            elif self.implementation == 1:
                self.pos_conv = nn.Conv2d(dim_intra_dimension, dim_k, (r, r), padding=(r // 2, r // 2))
        else:
