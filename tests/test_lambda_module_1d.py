...
def test_repr():
    layer = lambda_module_1d.LambdaLayer1D(64, 64, dim_k=16, m=32, heads=4, dim_intra=4)
...
def test_input_smaller_than_m(device, dim_intra):
    ...
    # This should work since t < m
    out = layer(x)
    assert out.shape == torch.Size([5, 64, 32])

    # m in layer = 64, m in input = 63
    x = torch.zeros(5, 64, 63, device=device)

    # This should work since t < m
    out = layer(x)
    assert out.shape == torch.Size([5, 64, 63])
